# copy from https://github.com/meituan-longcat/LongCat-Audio-Codec/blob/main/semantic_tokenizer_general/feature_extractor.py
import math
...
def generate_padding_mask_by_size(max_length, lengths, device="cpu"):
    ...
    mask = lengths[:, None] <= torch.arange(max_length,
                                            device=lengths.device)[None, :]
    mask = mask.to(device)
    return mask
...
class Fbank(nn.Module):
    ...
    def __init__(self, conf):
        ...
        self.window_shift = window_shift = int(self.freq * self.frame_shift /
                                               1000)
        self.window_size = window_size = int(self.freq * self.frame_length /
                                             1000)
        self.padded_window_size = padded_window_size = next_power_of_2(
            self.window_size)

        window = torch.hann_window(self.window_size,
                                   periodic=False,
                                   dtype=torch.float32).pow(0.85)
        ...
        mel = mel_scale(fft_bin_width *
                        torch.arange(num_fft_bins)).unsqueeze(0)
        ...
        self.pad_size = 0
        if hasattr(conf, "padded_num_mel_bins"):
    ...
    def forward(self, batch_wav, input_lens):
        ...
        with torch.no_grad():
            ...
            if wav_len < self.window_size:
                feats = torch.empty((b, 0, self.num_mel_bins),
                                    dtype=batch_wav.dtype,
                                    device=batch_wav.device)
                input_lens = torch.zeros(b,
            ...
            frames = batch_wav.as_strided((b, frame_len, self.window_size),
                                          (wav_len, self.window_shift, 1))
            if self.dither != 0.0:
                rand_gauss = torch.randn(frames.shape,
            ...
            if self.padded_window_size != self.window_size:
                ...
                frames = F.pad(frames, (0, padding_right),
                               mode="constant",
                               value=0)
            # spectrum
            ...
            input_lens = torch.div(input_lens - self.window_size,
                                   self.window_shift,
...
class Cmvn(nn.Module):
    ...
    def __init__(self, cmvn_file):
        ...
        cmvn_stats = np.load(cmvn_file)
        cmvn_count = cmvn_stats[0][-1]

        cmvn_mean, cmvn_var = calculate_mean_var(cmvn_stats[0][:-1],
                                                 cmvn_stats[1][:-1],
                                                 cmvn_count)
        cmvn_mean, cmvn_scale = calculate_mean_var_postprocess(
            cmvn_mean, cmvn_var)
...
class FeatureExtractor(nn.Module):
    ...
    def __init__(self, conf):
        ...
        self.fbank = Fbank(conf)
        self.cmvn = Cmvn(conf.cmvn)
